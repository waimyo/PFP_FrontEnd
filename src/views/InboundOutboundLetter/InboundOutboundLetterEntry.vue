    ...
    <v-dialog flat v-model="dialog" persistent max-width="800px">
      <v-card :disabled="isDisabled">
        <v-card-title class="grey black--text lighten-3">
          စာပေးပို့ခြင်း
          <v-spacer></v-spacer>
          <v-icon color="primary" text @click="dialog = false"
        ...
        <v-card-text>
          <v-container>
            ...
            >
              <v-row dense>
                <v-col cols="12" md="12">
                  <v-row v-if="cputouserlist" dense>
                    <v-col cols="12" md="3">
                      <v-subheader
                        >CPU Account<span class="red--text title"
                          >*</span
                        ></v-subheader
                      >
                    </v-col>
                  ...
                  <v-row v-if="disabletouserlist" dense>
                    <v-col cols="12" md="3">
                      <v-subheader
                        >Account<span class="red--text title"
                          >*</span
                        ></v-subheader
                      >
                    </v-col>
                  ...
                  <v-row dense>
                    <v-col cols="12" md="3">
                      <v-subheader
                        >Description<span class="red--text title"
                          >*</span
                        ></v-subheader
                      >
                    </v-col>
                    <v-col cols="12" md="7">
                      <v-textarea
                        v-model="inboxoutbox.description"
                        outlined
                        dense
                        rows="5"
...
export default {
  ...
  methods: {
    Insert() {
      var vm = this;
      if (
        this.inboxoutbox.receiver.length == 0 ||
        this.inboxoutbox.description == "" ||
        this.inboxoutbox.description == null
      ) {
        this.validate();
      } else {
        ...
        // }
        vm.isDisabled = true;
        if (vm.inboxoutbox.file == undefined) {
          vm.inboxoutbox.file = null;
        }
        InboxOutboxService.InsertData(vm.inboxoutbox).then((result) => {
          if (result.data.result.success) {
            ...
            if (vm.inboxoutbox.file != null) {
              ...
              InboxOutboxService.InsertAttachFile(
                vm.files,
                result.data.chatid
              ).then((result) => {
                console.log(result);
