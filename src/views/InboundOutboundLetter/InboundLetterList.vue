  ...
  <div>
    ...
    <div v-show="isExactActive">
      <v-col cols="12" md="12">
        ...
        <v-card>
          <v-card-title>ဝင်စာများ</v-card-title>
          <v-divider></v-divider>
          <v-col cols="12" md="12">
            ...
            >
              <template v-slot:item="row">
                ...
                <tr
                  :class="{ 'light-blue lighten-4': row.item.isread == false }"
                >
                  <td>
                    ...
                    <router-link
                      :to="{
                        path: '/InboundLetterList/InboundOutboundLetterDetail',
                        query: {
                          chatting_id: row.item.chat_id,
                          inbound: true,
                          sender_id: row.item.sender_id,
                        },
                      }"
                    >
                      ...
                      <v-badge
                        ...
                        v-if="row.item.countfornunread > 0"
                        :content="row.item.countfornunread"
                      >
                  ...
                  <!-- <td v-if="!ishidecreatebtn">{{ row.item.sender }}</td> -->
                  <td>{{ row.item.sender }}</td>
...
<script>
//import InboundOutboundLetterEntry from "./InboundOutboundLetterEntry.vue";
...
export default {
  ...
  created() {
    ...
    if (user.role_id == 2) {
      // this.headers = this.headerforMinistry;
  ...
  methods: {
    GetCPU() {
      var vm = this;
      DropDownService.GetCPUAccount().then((result) => {
    ...
    newItem() {
      ...
      if (this.disabletouserlist == true) {
        ...
        this.$refs.InboundOutboundLetterEntryModal.inboxoutbox.ismaindescription =
          this.disabletouserlist;
        this.$refs.InboundOutboundLetterEntryModal.disabletouserlist =
          this.disabletouserlist;
      }
      console.log(this.cputouserlist);
      if (this.cputouserlist == true) {
        this.$refs.InboundOutboundLetterEntryModal.touserlist = this.touserlist;
        console.log(this.touserlist);
        this.$refs.InboundOutboundLetterEntryModal.inboxoutbox.ismaindescription =
          this.cputouserlist;
        this.$refs.InboundOutboundLetterEntryModal.cputouserlist =
          this.cputouserlist;
      }
    ...
    GetAllData() {
      ...
      // }
      // params.sortBy = params.sortBy[0];
      InboxOutboxService.InboundList(params, this.fromdate, this.todate).then(
        (result) => {
