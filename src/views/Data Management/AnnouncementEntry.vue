                    ...
                    <v-form @submit.prevent="Insert" autocomplete="off" ref="form" v-model="valid" lazy-validation>
                        <v-row dense>
                            <v-col cols="12" md="12">
                                <v-row dense>
                                    <v-col cols="12" md="4">
                                        <v-subheader>Announcement<span class="red--text title">*</span></v-subheader>
                                    </v-col>
                                    <v-col cols="12" md="7">
                                        <v-textarea required :rules="Rules" v-model="announce.description" outlined dense></v-textarea>
                                    </v-col>
                                </v-row>
...
export default {
    data() {
        return {
            ...
            Rules: [(v) => !!v || "Announcement ထည့်ရန်လိုအပ်ပါသည်။"],
        };
    },
    computed: {},
    created() {
    ...
    methods: {
        ...
        Insert() {
            ...
            var vm = this;
            if (!vm.announce.description) {
                vm.validate();
            } else {
                vm.isDisabled = true;
                AnnouncementService.InsertData(vm.announce).then(
                    (result) => {
